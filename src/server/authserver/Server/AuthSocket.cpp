...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        if (_cmd == AUTH_LOGON_CHALLENGE)
        {
            ...
            {
				SF_LOG_DEBUG("server.authserver", "Got %u AUTH_LOGON_CHALLENGE in a row from '%s', possible ongoing DoS", challengesInARow, socket().getRemoteAddress().c_str());
                socket().shutdown();
                return;
            }
        }
        ...
        if (res2)
        {
            ...
            if (!locked)
            {
                ...
                else
                {
					...
					SF_LOG_DEBUG("server.authserver", "database authentication values: v='%s' s='%s'", databaseV.c_str(), databaseS.c_str());
...
void Patcher::LoadPatchMD5(char *szFileName)
{
	...
	SF_LOG_DEBUG("server.authserver", "Loading patch info from %s\n", path.c_str());
